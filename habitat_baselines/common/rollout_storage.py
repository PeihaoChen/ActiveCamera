...
class RolloutStorageOracle:
    ...
    def __init__(
        ...
        self.observations["curr_body_action"] = torch.zeros(
                num_steps+1, 
                num_envs,
                1, dtype=torch.int64
            )  

        policy_input = config.body_policy_inputs + config.camera_policy_inputs
        if "he_exp_area" in policy_input or "he_relative_angle" in policy_input or "all_he_inputs"  in policy_input:
            self.observations['one_hot_vector'] = torch.zeros(
                num_steps+1, 
                num_envs,
                config.circle_point_num, dtype=torch.float  
            )
            self.observations['heuristic_goal'] = torch.zeros(
                num_steps+1, 
                num_envs,
                2, dtype=torch.float
            )  
            self.observations['all_points_infos'] = torch.zeros(
                num_steps+1, 
                num_envs,
                config.circle_point_num, 
                4,
                dtype=torch.float
            )  
    ...
    def insert(
        ...
        for sensor in observations:
            if sensor == "curr_body_action" or sensor == "heuristic_goal" or sensor == "one_hot_vector"\
                or sensor == "all_points_infos":
                self.observations[sensor][self.step].copy_( # 因为现在insert的bodyActions是由上次insert的RGBD等信息规划得到的，而上一时刻的RGBD已经被存在self.step中
                    observations[sensor]
                )
            else:
                self.observations[sensor][self.step + 1].copy_(
