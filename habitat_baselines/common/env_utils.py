...
def make_env_fn(
    ...
    if len(config.TASK_CONFIG.specified_episode_id) != 0:   # 指定部分episode用于调试
        assert config.NUM_PROCESSES == 1, print("MUST EQUAL TO 1")
        dataset = dataset.filter_episodes(lambda x: int(x.episode_id) in config.TASK_CONFIG.specified_episode_id)
        logger.info("Specify {} episodes from original for experiment".format(dataset.num_episodes))
