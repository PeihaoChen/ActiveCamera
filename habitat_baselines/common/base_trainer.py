...
class BaseRLTrainerOracle(BaseTrainer):
    ...
    def eval(self) -> None:
        ...
        with TensorboardWriter(
            ...
            if os.path.isfile(self.config.EVAL_CKPT_PATH_DIR) or self.config.EVAL_CKPT_PATH_DIR == "random":
                # evaluate singe checkpoint or eval a agent which does not need to train
                self._eval_checkpoint(self.config.EVAL_CKPT_PATH_DIR, writer)
            else:
                # evaluate multiple checkpoints in order
                prev_ckpt_ind = -1
    ...
    def _pause_envs(
        ...
        if len(envs_to_pause) > 0:
            ...
            # indexing along the batch dimensions
            if body_recurrent_hidden_states is not None:
                body_recurrent_hidden_states = body_recurrent_hidden_states[
                    :, state_index
                ]
            if camera_recurrent_hidden_states is not None:
                camera_recurrent_hidden_states = camera_recurrent_hidden_states[
                    :, state_index
                ]
            not_done_masks = not_done_masks[state_index]
            current_episode_reward = current_episode_reward[state_index]
            current_episode_reward_exp_area = current_episode_reward_exp_area[state_index]
            current_episode_reward_exp_he = current_episode_reward_exp_he[state_index]
            prev_body_actions = prev_body_actions[state_index]
            prev_camera_actions = prev_camera_actions[state_index]
